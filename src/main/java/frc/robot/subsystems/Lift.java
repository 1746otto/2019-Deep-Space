...
public class Lift extends Subsystem {
  ...
  private double encTickToInches(double encTicks) {
    return ((((encTicks / Constants.kEncTicksPerInch)) + Constants.kElevatorInitialHeight) * Constants.kCascadingFactor);
  }

  private int inchesToEncTicks(double inches) {
    return (int) (((inches - Constants.kElevatorInitialHeight) / Constants.kCascadingFactor) * Constants.kEncTicksPerInch);
  }
  ...
  public boolean isSensorConnected() {
    int pulseWidthPeriod = master.getSensorCollection().getAnalogIn();
