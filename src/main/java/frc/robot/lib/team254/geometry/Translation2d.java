...
import frc.robot.lib.team254.util.Util;
...
public class Translation2d implements ITranslation2d<Translation2d> {
  ...
  public Translation2d translateBy(double distance, Rotation2d rotate) {
    double xTemp = Math.cos(Math.toRadians(rotate.getDegrees())) * distance;
    double yTemp = Math.sin(Math.toRadians(rotate.getDegrees())) * distance;

    return new Translation2d(x_ + xTemp, y_ + yTemp);
  }
  ...
  public Translation2d rotateBy(final Rotation2d rotation) {
    return new Translation2d(x_ * rotation.cos() - y_ * rotation.sin(), x_ * rotation.sin() + y_ 
        * rotation.cos());
  }
  ...
  public boolean epsilonEquals(final Translation2d other, double epsilon) {
    return Util.epsilonEquals(x(), other.x(), epsilon) && Util.epsilonEquals(y(), 
        other.y(), epsilon);
  }
   ...
   * @author Joseph Reed
    */  
  public boolean isWithinAngle(Translation2d A, Translation2d B, 
      Translation2d C, boolean vertical) {
  ...
  /**
   * Returns the angle between to Translational Vectors.
   * @param a Initial Translation Vector
   * @param b Secondary Translation Vector
